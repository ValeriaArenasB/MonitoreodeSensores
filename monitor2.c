...
#define PH_FLAG "PH"
#define END_OF_DATA -9999  //cuendo se acaban sensores, el monitor tiene que también que acabar

int BUFFER_SIZE;
sbuf_t buffer_temp, buffer_ph, buffer_fuera;
char *file_temp = NULL, *file_ph = NULL, *file_fuera= "file_fuera.txt";
...
void InicializarBuffers() {
    ...
    sem_init(&buffer_fuera.empty, 0, BUFFER_SIZE);
    sem_init(&buffer_fuera.full, 0, 0);
    sem_init(&buffer_fuera.mutex, 0, 1);
}
...
void LiberarBuffers() {
    ...
    sem_destroy(&buffer_fuera.empty); 
    sem_destroy(&buffer_fuera.full);
    sem_destroy(&buffer_fuera.mutex);
...
void *HiloTemp() {
    ...
    while (1) {
        ...
        if (temp >= 20.0 && temp <= 31.6) {
          ...
          sem_post(&buffer_fuera.mutex);
          sem_post(&buffer_fuera.empty);
...
void *HiloFuera(){
   ...
   FILE *file = fopen(file_fuera, "a");
    ...
    while (1) {
        sem_wait(&buffer_fuera.full);
        sem_wait(&buffer_fuera.mutex);

         ph = buffer_fuera.buf[buffer_fuera.out];
        buffer_fuera.out = (buffer_fuera.out + 1) % BUFFER_SIZE;

        sem_post(&buffer_fuera.mutex);
        sem_post(&buffer_fuera.empty);

        if (fuera == END_OF_DATA) {
            break;
        }
             strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S", tm_info);
